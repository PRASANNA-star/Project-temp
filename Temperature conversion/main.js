...
const toKelvin =celsius=>{
  let offset=celsius+273.15;
  document.getElementById("converted").innerHTML=
  Math.round(conversion*100)/100;
  if(conversion<=273.15){
    document.getElementById('converted').style.color='blue';
  }
  else if(conversion>=309.15){
    document.getElementById('converted').style.color='red';
  }
  else if(conversion>=273.15 && conversion<=309.15)
  {
    document.getElementById('converted').style.color='green';
  }
}
...
const convert = temp => {
  ...
  let kChecked = document.getElementById("kelvin-radio").checked;
  if (fChecked) {
    toFahrenheit(temp);
  } else if (cChecked) {
    toCelsius(temp);
